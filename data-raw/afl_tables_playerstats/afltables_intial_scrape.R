...
# Return list of raw afltables teams
# make fix.teams less opinionated
# 

get_afltables_ids <- function(seasons = as.integer(format(Sys.Date(), "%Y")), 
                              teams = unique(fitzRoy::fixture$Home.Team)){
  # Get htmls
  seas_htmls <- 
    seasons %>%
  ...
  home_teams <- 
    seas_htmls %>%
    purr::map(html_nodes, "td tr:nth-child(1) a:nth-child(1)")) %>%
    purr::map(html_text) %>%
    purr::map("%in%"(teams)
  ...
  game_urls <- purrr::map(~ rvest::html_nodes(.,"tr+ tr b+ a")) %>%
    purrr::map(~ rvest::html_attr(., "href")) %>%
    purrr::reduce(c) %>%
    purrr::map_chr(stringr::str_replace, "..", "https://afltables.com/afl")
  
  games_urls[home_teams | away_teams]
