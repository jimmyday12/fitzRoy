#' Fetch VFLW match results for a given season
#'
#' Scrapes match results from https://vfl.aflwstats.com for a specified season.
#'
...
#'
#' @examples
#' fetch_vflw_results(2025)
fetch_vflw_results <- function(season = 2025) {
  url <- paste0("https://vfl.aflwstats.com/season/", season)
  ...
  results <- purrr::map_dfr(round_tables, function(tbl) {
    ...
    purrr::map_dfr(rows, function(row) {
      ...
      tibble::tibble(
        ...
        Venue = venue,
        MatchLink = paste0("https://vfl.aflwstats.com", match_link)
      )
    })
  })
