...
# ---- Abbreviation Table ----
vflw_team_abbr <- tibble(
  Team = c(
    "Carlton", "Collingwood", "Essendon", "Western Bulldogs", "Geelong", "Casey Demons",
    "North Melbourne", "Williamstown", "Port Melbourne", "Box Hill Hawks", "Southern Saints",
    "Darebin Falcons", "GWS Giants", "Sydney Swans"
  ),
  Abbr = c(
    "car", "col", "ess", "wbd", "gee", "csy",
    "kan", "wil", "ptm", "haw", "san",
    "dar", "gws", "syd"
  )
)
...
scrape_vflw_game <- function(game_url, season, round) {
...
# ---- URL Generator ----
generate_vflw_game_urls <- function(season = 2025) {
  results_df <- fetch_vflw_results(season)
  
  results_df <- results_df %>%
    left_join(vflw_team_abbr, by = c("AwayTeam" = "Team")) %>%
    rename(AwayAbbr = Abbr) %>%
    mutate(
      RoundNum = as.integer(str_extract(Round, "\\d+")),
      HomeAbbr = str_extract(MatchLink, "[^/]+$"),
      GameURL = glue("https://vfl.aflwstats.com/game/{season}-{RoundNum}-{HomeAbbr}-{AwayAbbr}")
    ) %>%
...
# ---- Main Fetch Function ----
fetch_vflw_game_stats <- function(season = 2025) {
  message(glue("Fetching VFLW game stats for {season}..."))
  urls_df <- generate_vflw_game_urls(season)
  
  stats <- map2_dfr(urls_df$GameURL, urls_df$RoundNum, function(url, rnd) {
    message("⏳ ", url)
    tryCatch(
      scrape_vflw_game(url, season, rnd),
      error = function(e) {
