...
fetch_coaches_votes(season = 2021, comp = "AFLM")
```
...
fetch_coaches_votes(season = 2021, comp = "AFLW")
```
...
fitzRoy:::aflm_coaches_votes %>%
  dplyr::subset(Home.Team=="Western Bulldogs" | Away.Team == "Western Bulldogs")
```
...
fitzRoy:::aflm_coaches_votes %>%
  dplyr::subset(Home.Team=="Western Bulldogs" & Round == 24)
```
...
fitzRoy:::aflw_coaches_votes %>%
  dplyr::subset(Home.Team=="Western Bulldogs" & Round == 9)
```
...
```{r calculate_coaches_votes, eval=eval_param, include=TRUE}
df <- data.frame(
