...
For this example we will use `results` data from [AFL Tables](afltables.com) and `fixture` data from [Footywire](footywire.com). While this is generally fine, it can cause issues with teams, dates, venues or various other data to be inconsistent. This example will try to show some ways to take that into account. 

```{r load_data, eval=FALSE, include=TRUE}
# Get data
results <- fitzRoy::fetch_results_afltables()
fixture <- fitzRoy::fetch_results_footywire(2019)
```
