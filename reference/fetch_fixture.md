...
`fetch_fixture` returns the Fixture for a given AFL Round. Internally,
it calls a corresponding `fetch_fixture_*` function that depends on the
source given. By default the source used will be the official AFL
website.

`fetch_fixture_afl()`, `fetch_fixture_footywire()`,
`fetch_fixture_squiggle()` can be called directly and return data from
...
- convert_date:
  ...
  logical, if TRUE, converts date column to date format instead of date
  time.
